...
public class Kampf {
	...
	private Einheit findeNaechstesZiel(int angreiferPosition) {
		...
		if (einheiten[angreiferPosition].lebtNoch() && !(einheiten[angreiferPosition] instanceof Schaf)) {
			for (int i = 0; i < einheiten.length; i++) {
				int index = i + angreiferPosition > einheiten.length ? i + angreiferPosition - einheiten.length
						: i + angreiferPosition;
				if (einheiten[angreiferPosition] instanceof Zwerg || einheiten[angreiferPosition] instanceof Mensch) {
					if (einheiten[index] instanceof Ork || einheiten[index] instanceof Goblin
							|| einheiten[index] instanceof Schaf) {
						return einheiten[index];
					}
				} else {
					if (einheiten[index] instanceof Mensch || einheiten[index] instanceof Zwerg
							|| einheiten[index] instanceof Schaf) {
						return einheiten[index];
					}
				}
	...
	// hat.
	// Die Rückgabe ist die siegreiche Einheit, die den letzten Schlag
	// vollführt hat.
